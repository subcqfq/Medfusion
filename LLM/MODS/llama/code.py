...
def get_response(user_input):
    client = OpenAI(
        ...
        base_url="xxxxx",  # This uses Alibaba Cloud's LLM. If you need another platform, consult its docs and modify accordingly.
    )
    messages = [
    ...
    completion = client.chat.completions.create(
        model="llama-4-maverick-17b-128e-instruct",  # Here deepseek-r1 is used as an example; you can change the model name as needed.
        messages=messages
    )
...
def build_prompt(i):
    ...
    return (
        ...
        f"pH {ph}; arterial carbon dioxide partial pressure (PaCO2) {pco2} mmHg; "
        f"GCS score {gcs}; GCS motor {gcs_motor}; GCS verbal {gcs_verbal}; GCS eyes {gcs_eyes}; "
        f"PaO2/FiO2 ratio {pao2fio2ratio}; platelet count {platelet} K/uL; prothrombin time {pt} seconds; "
...
if __name__ == "__main__":
    workers = 1  # Parallelism per batch
