# %%
import pandas as pd
df = pd.read_csv('mimicmodstest.csv')
...
def get_response(user_input):
    client = OpenAI(
        ...
        base_url="https://dashscope.aliyuncs.com/compatible-mode/v1",  # This uses Alibaba Cloud's LLM; if you need another platform, consult its docs and modify accordingly.
    )
    messages = [
    ...
    messages.append({'role': 'user', 'content': 'Output only a single numeric value indicating the probability of hemorrhagic shock within the next 6 hours, without a percent sign, and nothing else.'})
    completion = client.chat.completions.create(
        model="deepseek-r1",  # deepseek-r1 is used here as an example; you can change the model name as needed.
        messages=messages
    )
...
def build_prompt(i):
    ...
    return (
        f"There is a {age}-year-old {gender} patient: heart rate {heart_rate} bpm, "
        ...
        f"What is this patient's probability of developing multiple organ dysfunction syndrome (MODS) in the next 24 hours? "
        f"Provide a detailed analysis, and you must also provide the probability."
    )
...
if __name__ == "__main__":
    workers = 110  # Parallelism per batch
