...
def load_and_preprocess_data():
    train_df = pd.read_csv('mimichxtrain.csv')
    test_df  = pd.read_csv('mimichxtest.csv')

    print(f"Train set size: {train_df.shape}")
    print(f"Test set size: {test_df.shape}")
    print(f"Train positive rate: {train_df['hx'].mean():.4f}")
    print(f"Test positive rate: {test_df['hx'].mean():.4f}")

    feature_columns = [c for c in train_df.columns if c != 'hx']

    X_train = train_df[feature_columns].values
    y_train = train_df['hx'].values.astype(np.float32)
    X_test  = test_df[feature_columns].values
    y_test  = test_df['hx'].values.astype(np.float32)
...
def main():
    ...
    for name, model in models_config.items():
        ...
        pd.DataFrame({"y_proba": test_preds}).to_csv(f"hx_{name}_proba.csv", index=False)
