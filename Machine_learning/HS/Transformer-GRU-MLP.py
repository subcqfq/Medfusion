...
def load_and_preprocess_data():
    train_df = pd.read_csv('mimichstrain.csv')
    test_df  = pd.read_csv('mimichstest.csv')

    print(f"Train set size: {train_df.shape}")
    print(f"Test set size: {test_df.shape}")
    print(f"Train positive rate: {train_df['hs'].mean():.4f}")
    print(f"Test positive rate: {test_df['hs'].mean():.4f}")

    feature_columns = [c for c in train_df.columns if c != 'hs']

    X_train = train_df[feature_columns].values
    y_train = train_df['hs'].values.astype(np.float32)
    X_test  = test_df[feature_columns].values
    y_test  = test_df['hs'].values.astype(np.float32)
...
def main():
    ...
    for name, model in models_config.items():
        ...
        pd.DataFrame({"y_proba": test_preds}).to_csv(f"hs_{name}_proba.csv", index=False)
